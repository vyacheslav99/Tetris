...
class Board(QFrame):
    ...
    def stop(self):
        ...
        self.isPaused = False
        self.locked = False
        self.msg2Statusbar.emit(f'Game Over! Lines: {self.linesRemoved}')
        self.update()
    ...
    def calc_pos(self, centerTop, centerLeft, prew_pos=False):
        """ вычисляет реальные координаты всех точек фигуры на доске от заданной точки центра по смещениям фигуры """
        offsets = self.figure.prew_offsets if prew_pos and self.figure.prew_offsets else self.figure.offsets
    ...
    def check_pos(self, pos):
        ...
        for coord in pos:
            if coord[0] < 0 or coord[0] >= Board.BHeight or coord[1] < 0 or coord[1] >= Board.BWidth \
                or self.board[coord[0]][coord[1]] < 0:
                return False
    ...
    def try_move(self, newRow, newCol):
        ...
        try:
            ...
            self.locked = True
            new_pos = self.calc_pos(newRow, newCol)
            curr_pos = self.calc_pos(self.figRow, self.figCol,
                                     prew_pos=(newCol == self.figCol and newRow == self.figRow))

            if not self.check_pos(new_pos):
    ...
    def paintEvent(self, event):
        for i in range(Board.BHeight):
            for j in range(Board.BWidth):
                self.draw_square(j * self.scale_width(), i * self.scale_height(), abs(self.board[i][j]))
